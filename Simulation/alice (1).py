...
# 1. Alice génère sa clé publique
alice_public_graph = protocol.alice_key_generation()
print("Alice a généré sa clé publique")

# Connexion au serveur (Bob)
server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server_socket.connect(('localhost', 12345))

# 2. Alice envoie sa clé publique à Bob
alice_public_graph_data = pickle.dumps(alice_public_graph)
server_socket.sendall(alice_public_graph_data)
print("Alice a envoyé sa clé publique à Bob")

# 3. Alice attend la clé publique de Bob
bob_public_graph_data = server_socket.recv(4096)
bob_public_graph = pickle.loads(bob_public_graph_data)
print("Alice a reçu la clé publique de Bob")

# 4. Alice génère la clé partagée
alice_shared_key = protocol.alice_compute_shared_key(bob_public_graph)
